...
func serverUnderTest(t *testing.T, devRoles ...identityentity.Role) *httptest.Server {
	...
	registry, err := secrets.NewRegistry([]*secrets.Store{
		secrets.NewStore(config.StoreConfig{
			ID:    "vault",
			Kind:  "keyway",
	...
	codec, err := transport.NewCodec(transport.GenerateKey())
	require.NoError(t, err)

	tokenService := tokens.NewService(newFakeTokenRepo())
	identityService := identity.NewService(&fakeIdentityRepo{}, nil)

	state := &transport.State{
		Stores:   registry,
		Access:   access.NewService(newFakeAccessRepo()),
		Audit:    audit.NewService(&fakeAuditRepo{}),
		Tokens:   tokenService,
		Identity: identityService,
		Auth: &transport.Auth{
			Tokens:   tokenService,
			Identity: identityService,
			Dev:      &transport.DevActor{Handle: "dev", Roles: devRoles},
			Codec:    codec,
		},
