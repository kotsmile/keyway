...
func newWorld(t *testing.T) *world {
	...
	return &world{
		...
		auditLog: auditLog,
		audit:    audit.NewService(auditLog),
		tokens:   tokens.NewService(newFakeTokenRepo()),
		identity: identity.NewService(&fakeIdentityRepo{}, nil),
		codec:    codec,
	}
}
...
func (w *world) serverAs(
	...
	state := &transport.State{
		...
		Auth: &transport.Auth{
			Tokens:   w.tokens,
			Identity: w.identity,
			Dev:      &transport.DevActor{Handle: handle, Roles: roles, Groups: groups},
			Codec:    w.codec,
		},
