...
func TestStatusMappingMatchesTheRustServer(t *testing.T) {
	...
	tests := []struct {
			...
			http.StatusInternalServerError, "internal error"},
		{
			...
			&secrets.NotAllowedError{Store: "prod", Verb: "delete"},
			http.StatusConflict,
			(&secrets.NotAllowedError{Store: "prod", Verb: "delete"}).Error(),
		},
		{
			// StoreError::Protected → Refused → 409.
			"a secret a reconciler owns",
			&secrets.ProtectedError{Name: "db-password", Marker: "argocd.argoproj.io/instance"},
			http.StatusConflict,
			(&secrets.ProtectedError{Name: "db-password", Marker: "argocd.argoproj.io/instance"}).Error(),
		},
		{
