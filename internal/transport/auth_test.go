...
// fakeTokenRepo is tokens storage in a map — the auth branching under test
...
func (r *fakeTokenRepo) Touch(_ context.Context, _ string, _ time.Time) {}
...
func newFakeIdentityRepo() *fakeIdentityRepo {
	return &fakeIdentityRepo{users: map[string]identityentity.RememberedUser{}}
}
...
// still.
func authUnderTest(t *testing.T, dev *DevActor) (*Auth, *tokens.Service, *fakeIdentityRepo) {
	t.Helper()
	tokenService := tokens.NewService(newFakeTokenRepo())
	users := newFakeIdentityRepo()
	return &Auth{
		Tokens:   tokenService,
		Identity: identity.NewService(users, nil),
		Dev:      dev,
		Codec:    testCodec(t),
		Now:      time.Now,
	}, tokenService, users
