// Package http is how keyway is reached over HTTP.
//
// It carries the Rust crate's transport module and its AppState: the error
// vocabulary, the session cookie, the auth middleware, the State every
// handler is given, and the handlers themselves — one file per domain
...
type State struct {
	Stores   *secretsservice.Registry
