// Package transport is signing in and out.
package transport
...
// Mount registers the sign-in routes on the root router. Sign-in lives at the
// root, not under /api: a browser is redirected here, and /api is for things
// that speak JSON.
func Mount(root chi.Router, state *transport.State) {
	root.Get("/auth/login", transport.Handler(login(state)))
	root.Get("/auth/callback", transport.Handler(callback(state)))
	root.Get("/auth/logout", logout)
}

// MountAPI registers who-am-I on the authenticated API router.
func MountAPI(api chi.Router, state *transport.State) {
	api.Get("/me", transport.Handler(me(state)))
}
...
func login(state *transport.State) func(http.ResponseWriter, *http.Request) error {
	return func(w http.ResponseWriter, r *http.Request) error {
		if state.Oidc == nil {
			return transport.BadRequest("this deployment has no issuer configured")
		}
		...
		http.SetCookie(w, transport.SealedCookie(state.Codec, pendingCookie, payload, 10*time.Minute))
...
func callback(state *transport.State) func(http.ResponseWriter, *http.Request) error {
	return func(w http.ResponseWriter, r *http.Request) error {
		if state.Oidc == nil {
			return transport.BadRequest("no issuer configured")
		}
		query := r.URL.Query()

		if issuerError := query.Get("error"); issuerError != "" {
			// The issuer refused. Its wording is the useful part.
			return transport.BadRequest(fmt.Sprintf("sign-in refused: %s", issuerError))
		}
		...
		if !opened {
			return transport.BadRequest("this sign-in expired; start again")
		}
		...
		if query.Get("state") != p.CSRF {
			return transport.BadRequest("state did not match")
		}
		code := query.Get("code")
		if code == "" {
			return transport.BadRequest("no code returned")
		}
		...
		http.SetCookie(w, transport.RemovalCookie(pendingCookie))
...
func logout(w http.ResponseWriter, r *http.Request) {
	http.SetCookie(w, transport.RemovalCookie(transport.SessionCookie))
	http.Redirect(w, r, "/", http.StatusSeeOther)
}
...
func me(state *transport.State) func(http.ResponseWriter, *http.Request) error {
	return func(w http.ResponseWriter, r *http.Request) error {
		actor := transport.Caller(r.Context())
		transport.WriteJSON(w, meView{
			Handle:    actor.Handle(),
