...
type cacheEntry struct {
	at     time.Time
	answer *identity.DirectoryAnswer
}
...
func (d *KeycloakDirectory) cached(handle string) (*identity.DirectoryAnswer, bool) {
	d.mu.Lock()
	defer d.mu.Unlock()
	...
	// request reads.
	return &identity.DirectoryAnswer{
		Enabled: entry.answer.Enabled,
		Groups:  slices.Clone(entry.answer.Groups),
	}, true
}

func (d *KeycloakDirectory) remember(handle string, answer *identity.DirectoryAnswer) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
// Resolve implements identity.Directory.
func (d *KeycloakDirectory) Resolve(ctx context.Context, handle string) (*identity.DirectoryAnswer, error) {
	...
	if !user.Enabled {
		d.remember(handle, &identity.DirectoryAnswer{Enabled: false})
		return &identity.DirectoryAnswer{Enabled: false}, nil
	}
	...
	d.remember(handle, &identity.DirectoryAnswer{
		Enabled: true,
